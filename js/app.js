...
function translateSystemStatusMessage(status) {
  ...
  if (code === "DEADLINE_CLOSED") {
    const deadlineText = String(status?.deadlineText || "").trim();

    if (deadlineText) {
      return t("deadlineClosedWithTime").replace("{deadline}", deadlineText);
    }

    return t("deadlineClosed");
  }
